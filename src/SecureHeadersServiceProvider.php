...
class SecureHeadersServiceProvider extends ServiceProvider
{
    ...
    protected function bootLaravel()
    {
        if ($this->app->runningInConsole()) {
            $this->publishes([
                $this->configPath() => config_path('secure-header.php'),
            ], 'config');
        }
    }
    ...
    {
        $this->app->configure('secure-header');
    }
    ...
    {
        $this->mergeConfigFrom($this->configPath(), 'secure-header');
    }
    ...
    {
        return __DIR__ . '/../config/secure-header.php';
    }
}
